...
class newsmsgcreateCommand extends commando.Command {
    constructor(client) 
    {
      super(client, {
        name: 'newsmsgcreate', 
        group: 'network',
        memberName: 'newsmsgcreate',
    ...
    async run(message, args)
    {
        ...
        if (!msgmsg) return message.channel.send("Usage: .newsmsgcreate <**message**>");

        if (!message.member.roles.has(staffrole.id)) return message.channel.send("Insufficient permission");

        var sendchannel = message.guild.channels.find(`name`, "news");

        sendchannel.send(msgmsg);
    }
}

module.exports = newsmsgcreateCommand;
