...
class givemsgcreateCommand extends commando.Command {
    constructor(client) 
    {
      super(client, {
        name: 'givemsgcreate', 
        group: 'network',
        memberName: 'givemsgcreate',
    ...
    async run(message, args)
    {
        ...
        if (!msgmsg) return message.channel.send("Usage: .givemsgcreate <**message**>");

        if (!message.member.roles.has(staffrole.id)) return message.channel.send("Insufficient permission");

        var sendchannel = message.guild.channels.find(`name`, "giveaways");

        sendchannel.send(msgmsg);
    }
}

module.exports = givemsgcreateCommand;
